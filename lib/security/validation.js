...
const CONTACT_FIELDS = new Set(["name", "need", "message", "website", "formStartedAt"]);
const SLIDE_FIELDS = new Set([
  "id", "label", "image", "kicker", "status", "title", "copy", "primary", "secondary",
  "primaryHref", "secondaryHref", "product",
]);
const APPROVED_LINK_HOSTS = new Set(["bizyako.com", "www.bizyako.com", "bizyako.vercel.app", "x.com", "www.x.com"]);
...
function validateContactPayload(payload) {
  ...
  if (!name || !need || !message) return failure("Please check the submitted details.");

  return { ok: true, value: { name, need, message }, message: "Request accepted." };
}
