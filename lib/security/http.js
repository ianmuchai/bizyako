...
const CANONICAL_PRODUCTION_ORIGINS = ["https://bizyako.com", "https://www.bizyako.com"];
...
function resolveAllowedOrigins(configuredOrigins, {
  ...
  const configured = normalizeAllowedOrigins(configuredOrigins);
  const candidates = production
    ? [...CANONICAL_PRODUCTION_ORIGINS, ...configured]
    : configured.length
      ? configured
      : normalizeAllowedOrigins(developmentOrigins);
