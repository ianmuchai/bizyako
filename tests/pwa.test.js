...
test("public assets and shell cache are versioned for reliable updates", () => {
  ...
  assert.match(homepage, /styles\.css\?v=20260813-1/);
  assert.match(homepage, /script\.js\?v=20260813-1/);
  assert.match(homepage, /chat-history\.js\?v=20260813-1/);
  assert.match(demo, /styles\.css\?v=20260813-1/);
  assert.match(demo, /product-demo\.js\?v=20260813-1/);
  assert.match(admin, /styles\.css\?v=20260813-1/);
  assert.match(admin, /admin\.js\?v=20260813-1/);
  assert.match(worker, /bizyako-shell-v12/);
  assert.match(worker, /20260813-1/);
  assert.ok(worker.includes("chat-history.js?v=20260813-1"));
  ...
  publicFiles.forEach((source) => {
    assert.doesNotMatch(source, /bizyako-shell-v11|20260805-1|bizyako-shell-v9|20260802-4|bizyako-shell-v8|20260802-3|bizyako-shell-v7|20260802-2|bizyako-shell-v6|20260802-1|bizyako-shell-v5|20260731-2/);
  });
});
