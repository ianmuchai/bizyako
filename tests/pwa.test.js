...
test("public assets and shell cache are versioned for reliable updates", () => {
  ...
  assert.match(homepage, /styles\.css\?v=20260802-2/);
  assert.match(homepage, /script\.js\?v=20260802-2/);
  assert.match(demo, /styles\.css\?v=20260802-2/);
  assert.match(demo, /product-demo\.js\?v=20260802-2/);
  assert.match(admin, /styles\.css\?v=20260802-2/);
  assert.match(admin, /admin\.js\?v=20260802-2/);
  assert.match(worker, /bizyako-shell-v7/);
  assert.match(worker, /20260802-2/);
  publicFiles.forEach((source) => {
    assert.doesNotMatch(source, /bizyako-shell-v6|20260802-1|bizyako-shell-v5|20260731-2/);
  });
});